...
import { X, CheckCircle2, Clock, AlertTriangle, ArrowUpRight } from "lucide-react";
...
export function Badge({ status, depositPercent }: { status: OrderStatus; depositPercent?: number }) {
  const s = STATUS_MAP[status];
  const label = status === "deposit" && depositPercent ? `${s.label} (${depositPercent}%)` : s.label;
  return (
    <span
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl p-5 border border-white/[0.06] bg-card group hover:border-white/10 transition-all duration-300">
      <div
        ...
        style={{ background: `radial-gradient(circle at 0% 0%, ${color}08 0%, transparent 60%)` }}
      />
      <div className="flex items-start justify-between mb-4">
        ...
        {trend && (
          <span className="flex items-center gap-0.5 text-[11px] font-medium" style={{ color: trend.startsWith("-") ? RED : GREEN }}>
            <ArrowUpRight size={12} style={trend.startsWith("-") ? { transform: "rotate(90deg)" } : {}} />
            {trend}
          </span>
        )}
      </div>
...
export function Modal({
  title, subtitle, onClose, children, width = 560,
}: {
...
export function GhostButton({ children, onClick, type = "button" }: { children: React.ReactNode; onClick?: () => void; type?: "button" | "submit" }) {
  return (
    <button
      type={type}
      onClick={onClick}
