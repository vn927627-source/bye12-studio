...
export function Orders() {
  ...
  const filtered = data.orders.filter((o) => {
    ...
    return !q || o.id.toLowerCase().includes(q) || classOf(o.customerId).toLowerCase().includes(q);
  });
  ...
  return (
    <div className="p-7 h-full overflow-y-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground">Quản lý Đơn thuê</h1>
          <p className="text-xs text-muted-foreground mt-0.5">{data.orders.length} đơn · {pendingCount} đơn chờ thanh toán đủ</p>
        </div>
        <button
        ...
        <div className="flex items-center gap-1 bg-card border border-white/[0.06] rounded-xl p-1">
          {TABS.map((t) => (
            <button
              ...
              style={tab === t.id ? { background: `linear-gradient(135deg, ${PINK}, #A855F7)`, color: "#fff" } : { color: "#7A6F8A" }}
            >
              {t.label}
            </button>
          ))}
      ...
      <div className="bg-card border border-white/[0.06] rounded-2xl overflow-hidden">
        {filtered.length === 0 ? (
          <p className="px-6 py-10 text-sm text-muted-foreground text-center">
            {data.orders.length === 0 ? 'Chưa có đơn thuê nào. Bấm "Tạo đơn thuê" để bắt đầu.' : "Không có đơn nào khớp bộ lọc."}
          </p>
        ) : (
            ...
            <tbody>
              {filtered.map((o) => (
                <tr key={o.id} className="border-t border-white/[0.04] group">
                  ...
                  <td className="px-6 py-3 text-muted-foreground">{o.maleCount} / {o.femaleCount}</td>
                  ...
                  <td className="px-6 py-3">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => setViewing(o)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-muted-foreground hover:text-foreground">
                        <Eye size={13} />
                      </button>
                      <button onClick={() => { setEditing(o); setShowModal(true); }} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-muted-foreground hover:text-foreground">
                        <Edit2 size={13} />
                      </button>
                      <button onClick={() => deleteOrder(o.id)} className="p-1.5 rounded-lg hover:bg-white/[0.06] text-muted-foreground hover:text-red-400">
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </td>
      ...
      {showModal && <OrderModal order={editing} onClose={() => setShowModal(false)} />}

      {viewing && (
        <Modal title={`Chi tiết đơn ${viewing.id}`} subtitle={classOf(viewing.customerId)} onClose={() => setViewing(undefined)} width={480}>
          <div className="space-y-3 text-sm">
            <Row label="Ngày thuê" value={viewing.rentDate} />
            <Row label="Ngày trả" value={viewing.returnDate || "Chưa xác định"} />
            <Row label="Số lượng nam / nữ" value={`${viewing.maleCount} / ${viewing.femaleCount}`} />
            <Row
              label="Đồ nam thuê"
              value={viewing.maleItems.map((i) => data.products.find((p) => p.id === i.productId)?.name).filter(Boolean).join(", ") || "—"}
            />
            <Row
              label="Đồ nữ thuê"
              value={viewing.femaleItems.map((i) => data.products.find((p) => p.id === i.productId)?.name).filter(Boolean).join(", ") || "—"}
            />
            <Row label="Tổng tiền" value={fmtMoney(orderTotal(viewing, data.products))} />
