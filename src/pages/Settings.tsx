...
export function SettingsPage() {
  ...
  function handleReset() {
    if (
      confirm(
        "Xóa toàn bộ dữ liệu (khách hàng, đơn thuê, kho đồ, ghi chú, học sinh)? Hành động này không thể hoàn tác.",
      )
    ) {
      resetAll();
    }
    ...
    <div className="p-7 h-full overflow-y-auto space-y-5 max-w-3xl">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Thông tin cửa hàng, mặc định tính giá, và dữ liệu
        </p>
      </div>
      ...
      <div className="bg-card border border-white/[0.06] rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Mô tả ngắn">
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, shopTagline: e.target.value })
              }
            />
          </Field>
        </div>
        ...
        <Field label="Ánh xạ size (tùy chọn)">
          <textarea
            className={inputCls}
            rows={4}
            placeholder='{"150-160": "S", "160-170": "M", ...}'
            value={JSON.stringify(form.sizeMapping || {}, null, 2)}
            onChange={(e) => {
              try {
                const mapping = JSON.parse(e.target.value);
                setForm({ ...form, sizeMapping: mapping });
              } catch {}
            }}
          />
          <p className="text-[11px] text-muted-foreground">
            Định dạng JSON: {"{"}"khoảng chiều cao-cân nặng": "Size"{"}"}
          </p>
        </Field>
      ...
      <div className="bg-card border border-white/[0.06] rounded-2xl p-6 space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          Toàn bộ dữ liệu được lưu ngay trên trình duyệt này. Xuất file để sao
          lưu hoặc chuyển sang máy khác.
        </p>
        <div className="flex gap-3">
          <GhostButton onClick={handleExport}>
            <Download size={14} /> Xuất dữ liệu (.json)
          </GhostButton>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">
          Xóa toàn bộ khách hàng, đơn thuê, kho đồ, lịch trình, ghi chú và học
